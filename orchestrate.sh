...
docker run -d --privileged -v /dev/log:/dev/log --volumes-from pulp-data -p 443:443 -p 8080:80 -e APACHE_HOSTNAME=$(hostname) --name pulp-apache pulp/apache
...
# crane
docker run -d -p 80:80 --volumes-from pulp-data --name pulp-crane pulp/crane-all-in-one
